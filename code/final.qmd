...
In part one they used a generalized linear model (specifically a logistic regression). The provided p-value indicates the predictor accurately explains the variance in the response.

In part two they used a Chi-square test to see if there was a relationship between the two categorical variables. The p-value indicates that there is a relationship between the two.

## b.

Observed American Avocets (*Recurvirostra americana*) were more likely to utilize micro-habitats closer to the water's edge. This suggests that proximity to water is a key feature of habitat suitability for this species.

The three observed shorebird species (American Avocets, Forster’s Tern, Black-necked Stilt) were not evenly distributed across habitats (managed pond, non-tidal marsh, salt pond, tidal marsh, or tidal mudflat). This means that each species tends to favor a specific habitat or multiple habitats, had there instead been even or completely random distribution we would not be able to assume this.

## c.

- An additional variable that we could collect would be percent vegetation coverage in each micro-habitat.
- This would be measured as a continuous variable with values ranging from 0% to 100%
- This variable could influence American Avocet micro-habitat choice as more coverage could provide more protection from predators or could conversely reduce foraging efficiency for the birds due to the density of foliage. Data collection for this would be fairly easy by either using quadrats at each habitat site (and collected each plant species + coverage) or by taking pictures of each habitat and using software like ImageJ to see the extent of coverage.
...
Simulated residuals look good, QQ has very little deviation from expected and residual vs predicted looks random with no discernible pattern.
